...
class SpawnServer(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(SpawnTurtle, "spawn_turtle", self.handle_request, callback_group = self.cb_group)
        self.spawn_cli = self.create_client(Spawn, "/spawn")

    async def handle_request(self, req, resp):
        if not (0 <= req.x <= 11 and 0 <= req.y <= 11):
            resp.success = False
            resp.message = "Coords out of range!"
            return resp

        spawn_req = Spawn.Request()
        spawn_req.x, spawn_req.y = float(req.x), float(req.y)
